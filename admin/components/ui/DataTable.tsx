      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            placeholder="Search..."
            className="admin-input pl-9 h-9 text-sm"
          />
        </div>
      ...
      <div className="hidden lg:block glass-card overflow-hidden">
        {sorted.length === 0 ? (
          <div className="py-20 text-center">
            <p className="text-white/20 text-lg font-semibold">{emptyMessage}</p>
            <p className="text-white/10 text-sm mt-2">{emptyDescription}</p>
          </div>
        ) : (
          <>
      ...
      <div className="lg:hidden space-y-3">
        {paginated.length === 0 && (
          <div className="glass-card py-16 text-center">
            <p className="text-white/20 text-sm font-semibold">{emptyMessage}</p>
            <p className="text-white/10 text-xs mt-1">{emptyDescription}</p>
          </div>
        )}
        {paginated.map((row) => (
          <motion.div
